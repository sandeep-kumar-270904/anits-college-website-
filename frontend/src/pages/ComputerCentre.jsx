...
const ComputerCentre = () => {
  ...
  const gallery = [
    '/images/computer-gallery-1.jpg',
    '/images/computer-gallery-2.jpg',
    '/images/computer-gallery-3.jpg',
    '/images/computer-gallery-4.jpg'
  ];

  return (
    <div className="font-sans pt-[52px] min-h-screen bg-gray-50 pb-16">
      <Helmet>
        <title>Computer Centre | ANITS</title>
      </Helmet>
      ...
      <div className="max-w-5xl mx-auto px-6 py-12 text-center">
        <h2 className="text-xl font-bold text-[#112a46] mb-12 text-left">Computer Centre</h2>
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-10">
          {gallery.map((img, idx) => (
            <img 
              key={idx}
              src={img} 
              alt={`Computer Centre Gallery ${idx + 1}`}
              onClick={() => setZoomedImage(img)}
              className="w-48 h-32 object-cover cursor-pointer rounded border border-gray-200 hover:shadow-lg transition-shadow"
              onError={(e) => {
                e.target.onerror = null;
                e.target.src = `https://placehold.co/400x300/e2e8f0/475569?text=Upload+computer-gallery-${idx+1}.jpg`;
              }}
            />
          ))}
        </div>
