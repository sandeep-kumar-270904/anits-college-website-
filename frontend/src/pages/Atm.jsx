...
const Atm = () => {
  ...
  const gallery = [
    '/images/atm-gallery-1.jpg',
    '/images/atm-gallery-2.jpg'
  ];

  return (
    <div className="font-sans pt-[52px] min-h-screen bg-gray-50 pb-16">
      <Helmet>
        <title>ATM | ANITS</title>
      </Helmet>
      ...
      <div className="max-w-5xl mx-auto px-6 py-12 text-center">
        <h2 className="text-xl font-bold text-[#112a46] mb-6 text-left">ATM</h2>
        
        <div className="text-gray-700 text-left mb-12">
          <p>Two ATM's are located inside the Campus.The Dhanalakshmi bank ATM is situated on the first floor of the Institute main building.The ICICI ATM is at institute entrance main gate.</p>
        </div>
        ...
        <div className="flex justify-center gap-4 mb-10">
          {gallery.map((img, idx) => (
            <img 
              key={idx}
              src={img} 
              alt={`ATM Gallery ${idx + 1}`}
              onClick={() => setZoomedImage(img)}
              className="w-48 h-64 object-cover cursor-pointer rounded border border-gray-200 hover:shadow-lg transition-shadow"
              onError={(e) => {
                e.target.onerror = null;
                e.target.src = `https://placehold.co/300x400/e2e8f0/475569?text=Upload+atm-gallery-${idx+1}.jpg`;
              }}
            />
          ))}
        </div>
